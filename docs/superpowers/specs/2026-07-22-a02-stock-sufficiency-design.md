...
前台 canonical pair 在每个 comparative run 都重新生成。每个 `L` 只能消费
`experiments/A0.2-foreground-length-qualification/anchors/foreground-<L>.json`：
`anchors/foreground-2048.json`、`anchors/foreground-8192.json`、
`anchors/foreground-16384.json`。只有同时满足：

```text
R0 cached = 0
R0/R1 semantic span、LCP 与 R1 prefix hash == per-L registered anchor
R1 cached is a valid integer, block-aligned, and 0 <= cached <= anchor target
```

每个 target 的无压力 admission 已由 foreground-length qualification 单独验证；它是 A0.2
baseline contract，不是压力后的每 run 通过条件。压力后的 block-aligned
`R1.cached < anchor target` 正是候选 GPU-prefix miss，只有在上列 token anchor 与计数边界都
成立时才可进入后续归因。缺失、非 block-aligned、超出 anchor target 或超过 prompt 长度的
计数才是 `accounting_contract_change`，暂停 matrix；它们不是 S0 miss、S1 restore 或性能数据。
...
hash 和 S1 所需 CPU GiB。对每个已提交 anchor，背景目标固定为：
...
若结果不为正整数，标记 `invalid_configuration`，而不是运行零 builder 的比较。它不提交 R1、
不产出 S0/S1 胜负、不运行 90-run matrix。
...
AB/BA is a pair-order attribute; it does not add a schedule dimension。
...
2. cached counter 缺失、非 block-aligned、超过对应 anchor target/实际 prompt 长度：
